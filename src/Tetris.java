...
public class Tetris extends JFrame {
	...
	PortReader serialComm;
	private static final float SPEEDCHANGE = 2.5f;
//	private static final float SPEEDCHANGE = 0.035f;	//old speedchange
	...
	private void updateGame() {
		...
		if(board.isValidAndEmpty(currentType, currentCol, currentRow + 1, currentRotation)) {
			...
			/*
			 * Increase/Decrease (depending on the EDA change) the speed noticeably for the 
			 * next piece and update the game's timer to reflect the increase.
			 */
			
			if(serialComm.isReady()) {
				if(serialComm.increasedEDA()) {
					if(gameSpeed < 5)
				...
				System.out.println("Increased: " + serialComm.increasedEDA());
			}
